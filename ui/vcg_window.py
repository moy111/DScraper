...
from web_scraper.vcg_scraper import VCGScraper  # 这里导入你的爬虫代码


# 创建爬虫线程
class VcgScraperThreadClass(QThread):
...
class VcgScraperApp(QWidget):
    ...
    def __init__(self, main_app):
        ...
        self.ui_elements = create_common_ui(
            self, "视觉中国", "访问视觉中国", self.open_webside, exclude_elements=["district_input", "max_links_input", "city_input"]
        )  # 加载公共 UI 模板

        # 修改文本
        self.ui_elements["page_label"].setText("爬取页数：(每一页133张图片)")

        # 修改启动按钮文本
        if self.ui_elements["start_button"]:
            self.ui_elements["start_button"].setText("开始爬取视觉中国图片")

        # 获取现有的 placeholder 文本
        existing_placeholder = self.ui_elements["log_placeholder"]
        # 在末尾添加一行新的文本
        new_placeholder = existing_placeholder + "\n此爬虫只能爬取800x大小无水印预览图片，更多原图请访问「视觉中国」官网下载"
    ...
    def start_scraper(self):
        ...
        started = scraper_utils.start_scraper(self, VcgScraperThreadClass)
    ...
    def open_webside(self):
        """打开 视觉中国 搜索页面"""
        webbrowser.open('https://www.vcg.com/')  # 修改为视觉中国的网址
