...
from web_scraper.huaban_scraper import Huabanscraper  # 这里导入你的爬虫代码


# 创建爬虫线程
class HuabanScraperThreadClass(QThread):
...
class HuabanScraperApp(QWidget):
    ...
    def __init__(self, main_app):
        ...
        self.ui_elements = create_common_ui(
            self, "花瓣网备份", "访问花瓣网", self.open_webside, exclude_elements=["page_input", "district_input", "max_links_input", "city_input"]
        )  # 加载公共 UI 模板

        # 修改 PlaceholderText 为花瓣网特定的提示信息
        if self.ui_elements["keyword_input"]:  # 确保 keyword_input 存在
            self.ui_elements["keyword_input"].setPlaceholderText("把用户主页链接粘贴到这里～例如：https://huaban.com/user/xxxx")

        # 修改关键词输入框的 Label 文本
        keyword_label = self.findChild(QLabel, "keyword_label")  # 使用 objectName 查找
        if keyword_label:
            keyword_label.setText("备份用户主页网页链接:")
...
# ---被动更新---
    ...
    def start_scraper(self):
        ...
        started = scraper_utils.start_scraper(self, HuabanScraperThreadClass)
    ...
    def open_webside(self):
        """打开 谷德 搜索页面"""
        webbrowser.open('https://huaban.com')  # 修改为花瓣网的网址
