...
![DScraper](https://qjdxx9wxscbnncpx.public.blob.vercel-storage.com/DScraper.png)
...
- **多网站支持**：集成多个设计和图片网站的爬虫模块
  ...
  - Gooood（设计网站）
  - 知末效果图（ZNZMO）  
  > 由于目标网站前端结构调整，当前爬虫模块可能需要更新选择器参数
- 花瓣网（Huaban）  
  > 由于网页结构变动，部分功能可能失效，欢迎提交修复或优化
- 大众点评（Dianping）  
  > 目标页面结构存在变更，相关爬虫逻辑需根据实际情况调整
- 小红书（Xiaohongshu）  
  > 平台页面更新较频繁，当前版本可能存在不兼容情况
